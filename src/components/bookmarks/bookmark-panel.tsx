...
type ImageMode = "PLACEHOLDER" | "UPLOAD";
    ...
    try {
      const response = await fetch("/api/bookmarks", {
        ...
        body: JSON.stringify({
          ...
          url,
          imageType: imageMode,
          imageValue: imageMode === "PLACEHOLDER" ? placeholder : uploadUrl
        })
      });
        ...
        <section>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            <button
              type="button"
              className={clsx(
                "rounded-lg border p-4 text-center transition focus:outline-none focus:ring-2 focus:ring-blue-500",
                imageMode === "PLACEHOLDER" ? "border-blue-500 bg-blue-50" : "border-slate-200 hover:bg-slate-50"
              )}
              onClick={() => setImageMode("PLACEHOLDER")}
            >
              <ImageIcon className="mx-auto mb-3 h-8 w-8 text-blue-600" />
              <span className="block text-sm font-semibold text-slate-900">Use Placeholder</span>
              <span className="mt-1 block text-xs text-slate-500">Choose from our library</span>
            </button>
            <button
              type="button"
              className={clsx(
            ...
            >
              ...
              <span className="mt-1 block text-xs text-slate-500">Upload your own image</span>
            </button>
          </div>
        </section>
